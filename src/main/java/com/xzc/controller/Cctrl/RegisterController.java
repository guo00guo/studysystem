...
public class RegisterController {
    ...
    public Result updateUserById(User user, MultipartFile file) {
        ...
        Integer i = accountService.updateUserById(user,file);

        if (i < 1) {
            return Result.error();
        }
        else{
            User newUser = accountService.selectUserById(user.getUserId());
    ...
    public Result changePassWord(Integer userId, String oldPassword, String newPassword) {
        ...
        Integer i = accountService.changePassWord(userId, oldPassword, newPassword);
        if (i < 1) {
